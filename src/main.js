...
//added----------------------------------
import { fetchPopularMovies } from "./network.js";
...
async function loadModal() {
  ...
  const notificationHTML = await (
    await fetch("components/notification.html")
  ).text();
  ...
  function closeModal() {
    ...
    document
      ...
      .classList.remove("opacity-100", "scale-100", "pointer-events-auto");
    document
      .getElementById("movieModal")
      .classList.add("opacity-0", "scale-95", "pointer-events-none");
...
fetchPopularMovies().then((movies) => {
  allMovies = movies;
  displayMovies(allMovies, moviesContainer, handleClick);
});
...
async function displayGenreFilters() {
  ...
  genres.forEach((genre) => {
    ...
    pill.className =
      "bg-gray-800 text-white text-sm px-4 py-2 rounded-full hover:bg-red-600 hover:shadow-xl shadow-(color: red) border-gray-400 border-[1px] transition pointer-cursor";
...
function filterMoviesByGenres() {
  ...
  //added---------------------!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  displayMovies(filtered, moviesContainer, handleClick);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (searchInput) {
    searchInput.addEventListener("input", (e) => {
      ...
      //added----------------------------------!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      displayMovies(searchResults, moviesContainer, handleClick);
    });
  }
});
//remove-----------------------------------------------------!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
